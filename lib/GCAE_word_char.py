...
from keras.layers import Bidirectional, LSTM, GRU, SpatialDropout1D
import pickle
import os
    ...
    def build_model(self):
        ...
        with tf.name_scope('gate-cnn'):
            ...
            for idx, a_aspect in enumerate(all_aspects):
                ...
                print('x:',x)
                for i in x:
                    print('i.getshape:',i.get_shape())
                    print('list:',i.get_shape().as_list())
                    print('stride:', i.get_shape().as_list()[1])
                x = [tf.squeeze(tf.layers.max_pooling1d(i, i.get_shape().as_list()[1], i.get_shape().as_list()[1], 'SAME')) for i in x]  # [(bs,kn), ...]*len(Ks)
                x = tf.concat(x, 1)
