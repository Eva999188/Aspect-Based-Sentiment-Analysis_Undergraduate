...
class SynAtt(object):
    ...
    def build_model(self):
        ...
        with tf.name_scope('dynamic_rnn'):
            ...
            if self.is_bi_rnn:
                context_outputs = Bidirectional(LSTM(self.n_hidden, return_sequences=True), merge_mode=None)(
                    context_inputs)
                context_outputs2 = context_outputs[1]
                context_outputs = context_outputs[0]
            else:
                context_outputs = LSTM(self.n_hidden, return_sequences=True)(context_inputs)
    ...
    def cal_f1(self, labels, predicts):
        ...
        for col_idx in range(self.n_class):
            crt_f1 = f1_score(labels[:, col_idx], predicts[:, col_idx], average='weighted')
            crt_mean_f1 = np.mean(crt_f1)
            # crt_mean_f1 = (crt_f1[0] + crt_f1[1] + crt_f1[3])/3
    ...
    def run(self, train_data, test_data, model_suffix='model_iter', max_to_keep=0):
        max_to_keep = 0
        ...
        for i in range(self.n_epoch):
            sub_val_cnt = 0
            # print('test1')
            while (len(train_data[0]) - sub_val_cnt * self.val_num > 120):
                # print('test2')
                print(len(train_data[0]),sub_val_cnt,self.val_num)
                ...
                # print('test4')
                model_path = 'models/%d/' % (i)
                if not os.path.isdir(model_path):
                    os.mkdir(model_path)
                model_name = 'models/%d/%s_iter_%d_%d' % (i,model_suffix, i, sub_val_cnt)
                ...
                if test_f1 > max_f1:
                    ...
                    if (crt_tol < self.early_stop):
                        ...
                        if step >= 0:
                            ...
                            saver.restore(self.sess, 'models/%d/%s_iter_%d_%d' % (step,model_suffix, step, sub_step))
